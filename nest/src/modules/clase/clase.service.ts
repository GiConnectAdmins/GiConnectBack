...
export class ClaseService {
  ...
  async update(id: string, dto: UpdateClaseDto) {
    ...
    const tieneDiaSemanaFinal =
      dto.diaSemana !== undefined ? Boolean(dto.diaSemana) : Boolean(claseExistente.diaSemana);
    const tieneFechaFinal =
      dto.fecha !== undefined ? Boolean(dto.fecha) : Boolean(claseExistente.fecha);

    if (tipoFinal === 'recurrente') {
      if (dto.fecha) {
        throw new BadRequestException('Una clase recurrente no puede tener el campo "fecha"');
      }
      if (!tieneDiaSemanaFinal) {
        throw new BadRequestException('Una clase recurrente debe tener el campo "diaSemana"');
      }
    }

    if (tipoFinal === 'especial') {
      if (dto.diaSemana) {
        throw new BadRequestException('Una clase especial no puede tener el campo "diaSemana"');
      }
      if (!tieneFechaFinal) {
        throw new BadRequestException('Una clase especial debe tener el campo "fecha"');
      }
    }
  ...
  private validarCoherencia(tipo: string, fecha?: string, diaSemana?: string) {
    if (tipo === 'recurrente') {
      ...
      if (fecha) {
        throw new BadRequestException('Una clase recurrente no debe tener el campo "fecha"');
      }
    }
    ...
    if (tipo === 'especial') {
      ...
      if (diaSemana) {
        throw new BadRequestException('Una clase especial no debe tener el campo "diaSemana"');
      }
    }
  }
}
