...
export class PersonService {
  ...
  async getById(id: string, usuarioActual: PersonDocument) {
    ...
    if (rol === 'Atleta') {
      ...
      const equipoPersonaId = (persona.equipo as unknown as { _id?: unknown })?._id;
      const mismoEquipo =
        equipoUsuario && equipoPersonaId && equipoUsuario.toString() === equipoPersonaId.toString();
  ...
  async updateMe(userId: string, dto: UpdateMeDto) {
    ...
    return this.personModel
      .findByIdAndUpdate(userId, camposPermitidos, { new: true, runValidators: true })
      .populate('equipo', 'nombre fotoLogo')
      .populate('cinturon', 'color grado');
  }
  ...
  async updateById(id: string, dto: UpdatePersonDto, usuarioActual: PersonDocument) {
    ...
    if (rolUsuario === 'Admin') {
      ...
      if (!equipoDelAtleta) {
        throw new ForbiddenException('Solo puedes modificar atletas de tu equipo');
      }
    ...
    return this.personModel
      .findByIdAndUpdate(id, camposPermitidos, { new: true, runValidators: true })
      .populate('equipo', 'nombre fotoLogo')
      .populate('cinturon', 'color grado');
  }
