...
import { Cinturon, CinturonDocument } from '../cinturon/schemas/cinturon.schema';
...
export class BeltDateService {
  constructor(
    @InjectModel(BeltDate.name) private readonly beltDateModel: Model<BeltDateDocument>,
    @InjectModel(Cinturon.name) private readonly cinturonModel: Model<CinturonDocument>,
  ) {}
  ...
  async getById(id: string) {
    const beltDate = await this.beltDateModel.findById(id).populate('cinturon', 'color grado');
    if (!beltDate) {
  ...
  async create(dto: CreateBeltDateDto) {
    ...
    const fechaFinal = dto.fecha ? this.validarFechaNoFuturaCreate(dto.fecha) : new Date();

    const nuevoBeltDate = new this.beltDateModel({ cinturon: dto.cinturon, fecha: fechaFinal });
  ...
  private validarFechaNoFuturaCreate(fecha: string): Date {
    ...
    if (fechaProporcionada > fechaActual) {
      throw new BadRequestException('No se puede registrar una fecha de concesión futura');
    }
