...
export class SolicitudEquipoService {
  ...
  async aceptar(
    id: string,
    usuarioActual: PersonDocument,
    dto: RespondSolicitudEquipoDto,
  ) {
    const solicitud = await this.obtenerSolicitudParaResponder(
      id,
      usuarioActual,
    );
   ...
   */
  private async obtenerSolicitudParaResponder(
    id: string,
    usuarioActual: PersonDocument,
  ) {
